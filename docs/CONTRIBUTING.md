...
3. **Add the original repository** as upstream:
   ```bash
   git remote add upstream https://github.com/LearnCodeWithRam/portfolio.git
   ```
...
5. **Set up environment variables**:
   ```bash
   # Create .env.local and add GEMINI_API_KEY, MISTRAL_API_KEY, and/or GROQ_API_KEY
   ```
...
- **Gemini, Mistral, and Groq APIs** - AI chat functionality with ordered fallback
