              ...
              >
                {questionConfig.map(({ key, color, icon: Icon }) => (
                  <Button
                    ...
                    variant="outline"
                    className="border-border hover:bg-border/30 bg-background/70 h-auto min-w-[100px] flex-shrink-0 cursor-pointer rounded-xl border px-4 py-3 shadow-none backdrop-blur-sm transition-none active:scale-95"
                  >
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <Drawer.Trigger className="group relative flex flex-shrink-0 items-center justify-center">
                        <motion.div
                          className="border-border hover:bg-border/30 bg-card/80 flex h-auto cursor-pointer items-center space-x-1 rounded-xl border px-4 py-3 text-sm backdrop-blur-sm transition-all duration-200"
                          whileHover={{ scale: 1 }}
                          whileTap={{ scale: 0.98 }}
                        >
                          <div className="text-foreground flex items-center gap-3">
          ...
          <Drawer.Content className="bg-background fixed right-0 bottom-0 left-0 z-100 mt-24 flex h-[80%] flex-col rounded-t-[10px] outline-none lg:h-[60%]">
            <div className="bg-background flex-1 overflow-y-auto rounded-t-[10px] p-4">
              <div className="mx-auto max-w-md space-y-4">
                <div
                  aria-hidden
                  className="bg-muted-foreground/40 mx-auto mb-8 h-1.5 w-12 flex-shrink-0 rounded-full"
                />
  ...
  return (
    <motion.button
      className={cn(
        ...
        'text-foreground hover:bg-muted/70 dark:hover:bg-muted/60',
        'transition-all',
        'focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500',
        isSpecial ? 'bg-black text-white' : 'bg-card'
      )}
      onClick={onClick}
