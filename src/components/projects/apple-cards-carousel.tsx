    ...
    >
      <div className="relative w-full">
        ...
        <div className="mr-10 flex justify-end gap-2 md:mr-20">
          <button
            className="bg-muted text-foreground relative z-40 flex h-10 w-10 items-center justify-center rounded-full disabled:opacity-50"
            onClick={scrollLeft}
            disabled={!canScrollLeft}
          >
            <IconArrowNarrowLeft className="text-muted-foreground h-6 w-6" />
          </button>
          <button
            className="bg-muted text-foreground relative z-40 flex h-10 w-10 items-center justify-center rounded-full disabled:opacity-50"
            onClick={scrollRight}
          ...
          >
            <IconArrowNarrowRight className="text-muted-foreground h-6 w-6" />
          </button>
        </div>
      </div>
    </CarouselContext.Provider>
...
export const Card = ({
  ...
  // @ts-expect-error The hook accepts a mutable ref for outside-click tracking.
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed inset-0 z-52 h-screen overflow-auto">
            ...
            <motion.div
              ...
              className="bg-card relative z-[60] mx-auto my-10 h-fit max-w-5xl rounded-3xl font-sans"
            >
              {/* Sticky close button */}
              <div className="sticky top-4 z-52 flex justify-end px-8 pt-8 md:px-14 md:pt-8">
                <button
                    className="bg-foreground text-background flex h-8 w-8 items-center justify-center rounded-full shadow-md"
                  onClick={handleClose}
                >
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="bg-card relative z-10 flex h-48 w-80 flex-col items-start justify-start overflow-hidden rounded-3xl"
      >
